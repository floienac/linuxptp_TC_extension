...
/*Pi slave data analysis -->*/
	//FILE *fd;
	//char *output_filename = "/home/user/Documents/florian/data.log";
	//time_t rawtime;
	//char time_str[50];
	//int case_nb = s->count;
/*<--Pi slave data analysis*/
...
/*Pi slave data analysis -->*/
	/*
	 * Print the offset, drift and ppb in file
	 * /!\ opens & closes output file for each measure !
	 * /!\ also print phc2sys sample if started...
	 */

	//if ((fd = fopen(output_filename, "a")) == NULL) {
	//	pr_err("impossible to open '%s' for slave data analysis", 
	//	       output_filename);
	//	exit(1);
	//}
	//time(&rawtime);
	//strncpy(time_str, ctime(&rawtime), 50);
	//time_str[strlen(time_str)-1]='\0';

	//fprintf(fd, "%s %d %" PRId64 " %f %f\n", time_str, case_nb, offset, s->drift, ppb);
	//printf("%s %d %" PRId64 " %f %f\n", time_str, case_nb, offset, s->drift, ppb);
	//fclose(fd);
/*<--Pi slave data analysis*/
