...
/*Transparent clock extension -->*/
		" -T        transparent clock mode, needs exactly 2 interfaces: "
		"	    -i <master> -i <slave> (overrides configuration file)\n"
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
		case 'T':
			if (config_set_int(cfg, "transparentClock", 1)) {
				goto out;
			}
			break;
/*<--Transparent clock extension*/
	...
	//clock = clock_create(cfg->n_interfaces > 1 ? CLOCK_TYPE_BOUNDARY : CLOCK_TYPE_ORDINARY, cfg, req_phc);
/*Transparent clock extension -->*/
	if (config_get_int(cfg, NULL, "transparentClock") && 
	    cfg->n_interfaces != 2) {
		fprintf(stderr, "transparent mode needs exactly 2 interfaces "
			"(master, slave), %d given\n", cfg->n_interfaces);
		usage(progname);
		goto out;
	}
