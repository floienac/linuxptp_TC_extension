					  ...
					  struct ptp_message *rsp, int id)
{
	...
	case TLV_SLAVE_ONLY:
		...
		break;
/*Transparent clock extension -->*/	
	case TLV_TRANSPARENT:
		mtd = (struct management_tlv_datum *) tlv->data;
		mtd->val = c->dds.flags & DDS_TRANSPARENT;
		datalen = sizeof(*mtd);
		respond = 1;
		break;
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	if (config_get_int(config, NULL, "transparentClock")) {
		c->dds.flags |= DDS_TRANSPARENT;
	}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
		if (config_get_int(config, NULL, "transparentClock")) {
			pr_err("TC mode does not allow one step");
			return NULL;
		}
/*<--Transparent clock extension*/
		switch (timestamping) {
...
/*Transparent clock extension -->*/
		/* Why PHC set to -1 ?
		 * We need no PHC switch in TC + free_running + UNCALIBRATED */
		phc_index = 0;
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
struct port *tc_port_by_side(enum tc_port_side side, struct clock *c)
{
	struct port *piter;

	if (clock_transparent(c)) {
		if (side != SIDE_UNKNOWN) {
			LIST_FOREACH(piter, &c->ports, list) {
				if (tc_port_side_get(piter) == side)
					return piter;
			}
			pr_err("port with given side not found");
		} else
...
/*Transparent clock extension -->*/	
void tc_residence_time_set(struct clock *c, tmv_t delay)
{
	if(clock_transparent(c))
		c->tc_residence_time = delay;
	else {
		pr_err("non transparent clock cannot have internal delay");
		exit(1);
	}
}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/	
tmv_t tc_residence_time_get(struct clock *c)
{
	return c->tc_residence_time;
}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
int clock_transparent(struct clock *c)
{
	return c->dds.flags & DDS_TRANSPARENT;
}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	if (c->free_running) {
		if (clock_transparent(c)) {
			pr_debug("TC in free running: "
				 "servo cannot be properly locked so forced");
			clock_no_adjust(c, ingress, origin);
			return SERVO_LOCKED;
		} else
			return clock_no_adjust(c, ingress, origin);
	}
...
/*Transparent clock extension -->*/
		/* Useless to run BMC in transparentClock mode: grand master is known. */
		if (clock_transparent(c)) {
			ps = PS_TRANSPARENT;			
		} else {
			ps = bmc_state_decision(c, piter);
...
/*Transparent clock extension -->*/
		case PS_TRANSPARENT:
			pr_notice("assuming the transparent clock role");
			clock_update_slave(c);
			event = EV_RS_TRANSPARENT;
			break;
/*<--Transparent clock extension*/
		default:
