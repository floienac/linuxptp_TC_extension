...
void msg_common_deep_memcpy(struct ptp_message *m_dest, struct ptp_message *m_src)
{
	/* Header deep memcpy */
	memcpy(&m_dest->header.tsmt, 
		&m_src->header.tsmt,
		sizeof(m_dest->header.tsmt));
	memcpy(&m_dest->header.ver, 
		&m_src->header.ver,
		sizeof(m_dest->header.ver));
	memcpy(&m_dest->header.messageLength, 
		&m_src->header.messageLength,
		sizeof(m_dest->header.messageLength));
	memcpy(&m_dest->header.domainNumber, 
		&m_src->header.domainNumber,
		sizeof(m_dest->header.domainNumber));
	memcpy(&m_dest->header.reserved1, 
		&m_src->header.reserved1,
		sizeof(m_dest->header.reserved1));
	memcpy(&m_dest->header.flagField, 
		&m_src->header.flagField,
		sizeof(m_dest->header.flagField));
	memcpy(&m_dest->header.correction, 
		&m_src->header.correction,
		sizeof(m_dest->header.correction));
	memcpy(&m_dest->header.reserved2, 
		&m_src->header.reserved2,
		sizeof(m_dest->header.reserved2));
	memcpy(&m_dest->header.sourcePortIdentity, 
		&m_src->header.sourcePortIdentity,
		sizeof(m_dest->header.sourcePortIdentity));
	memcpy(&m_dest->header.sequenceId, 
		&m_src->header.sequenceId,
		sizeof(m_dest->header.sequenceId));
	memcpy(&m_dest->header.control, 
		&m_src->header.control,
		sizeof(m_dest->header.control));
	memcpy(&m_dest->header.logMessageInterval, 
		&m_src->header.logMessageInterval,
		sizeof(m_dest->header.logMessageInterval));

	/* Other fields deep memcpy */
	memcpy(&m_dest->ts, 
		&m_src->ts,
		sizeof(m_dest->ts));
	memcpy(&m_dest->hwts, 
		&m_src->hwts,
		sizeof(m_dest->hwts));
	memcpy(&m_dest->address, 
		&m_src->address,
		sizeof(m_dest->address));
	memcpy(&m_dest->tlv_count, 
		&m_src->tlv_count,
		sizeof(m_dest->tlv_count));
	memcpy(&m_dest->last_tlv, 
		&m_src->last_tlv,
		sizeof(m_dest->last_tlv));
}
/*<--Transparent clock extension*/

/*Transparent clock extension -->*/
/* Msg copy not possible by pointers to structure copy: it's a shallow copy meaning that 
 * the data shared can be accessed by 2 pointers and is not safe */
int msg_deep_memcpy(struct ptp_message *m_src, struct ptp_message *m_dest)
{
	int type;

	msg_common_deep_memcpy(m_dest, m_src);
	type = msg_type(m_src);

	switch (type) {
	case SYNC:
		memcpy(&m_dest->sync.originTimestamp, 
		       &m_src->sync.originTimestamp,
		       sizeof(m_dest->sync.originTimestamp));
		return 0;
	case DELAY_REQ:
		pr_warning("function not available yet");
		break;
	case PDELAY_REQ:
		pr_warning("function not available yet");
		break;
	case PDELAY_RESP:
		...
		break;
	case FOLLOW_UP:
		memcpy(&m_dest->follow_up.preciseOriginTimestamp, 
		       &m_src->follow_up.preciseOriginTimestamp,
		       sizeof(m_dest->follow_up.preciseOriginTimestamp));
		break;
	case DELAY_RESP:
		pr_warning("function not available yet");
		break;
	...
	case MANAGEMENT:
		...
		break;
	default:
		pr_err("unknown message type: %d", type);
		return -1;
	}
	return 0;	
}
