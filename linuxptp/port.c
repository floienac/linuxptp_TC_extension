...
*/
/*Transparent clock extension -->*/
/* Enums moved to port.h */
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
/*
 * Residence time in TC measured in 2 steps: 
 * post recv (ingress) and pre send (departure)
 */
static void tc_residence_time(struct port *p,
			      struct timespec ts,
		   	      int first_step)
{
	tmv_t t, prev_delay, new_delay;

	if (!clock_transparent(p->clock)) {
		pr_err("clock should be transparent for "
		       "residence time measurement");
		exit(1);
	}
	
	prev_delay = tc_residence_time_get(p->clock);
	t = timespec_to_tmv(ts);

	if (first_step) {
		new_delay = t;
	} else {
		new_delay = tmv_sub(t, prev_delay);
		pr_debug("transparent clock residence time: %ld ns", new_delay);
		if (new_delay < 0) {
			pr_warning("negative residence time: %ld ns\n"
				   "\tt1=%ld\n"
				   "\tt2=%ld", 
				   new_delay, prev_delay, t);
			pr_warning("phc2sys probably not started: needed to "
				   "synchronize the 2 TC ports clocks");
		}
	}
	
	tc_residence_time_set(p->clock, new_delay);
}
/*<--Transparent clock extension*/

/*Transparent clock extension -->*/
/*
 * Forward a FOLLOW_UP message through the transparent clock.
 */
static int tc_forward_follow_up(struct port *p, 
			        struct ptp_message *m)
{
	struct port *outgoing_port;
	struct ptp_message *outgoing_msg;
	tmv_t sync_tc_residence_time;
	int err;

	outgoing_port = tc_other_side_port(p);

	/* correct origin ts with TC residence time of previous sync */
	sync_tc_residence_time = tc_residence_time_get(p->clock);
	if (tmv_is_zero(sync_tc_residence_time)) {
		pr_warning("TC residence time not set");
		goto out;
	}

	outgoing_msg = msg_allocate();
	if (!outgoing_msg)
		goto out;

	msg_deep_memcpy(m, outgoing_msg);

	outgoing_msg->header.correction += 
		tmv_to_correction(sync_tc_residence_time);
	/* peerMeanPathDelay already in timeInterval / correction value format */
	outgoing_msg->header.correction += 
		tmv_to_correction(TimeInterval_to_tmv(p->peerMeanPathDelay));

	pr_debug("follow_up correction field: %ld ns", 
		 correction_to_tmv(outgoing_msg->header.correction));

	//msg_print(m, stdout);

	/* Needed because preciseOriginTimestamp modified somewhere else... But WHY ??? */
	ts_to_timestamp(&outgoing_msg->ts.pdu, 
			&outgoing_msg->follow_up.preciseOriginTimestamp);

	err = port_prepare_and_send(outgoing_port, outgoing_msg, TRANS_GENERAL);
	if (err) {
		pr_warning("port %d to port %d: TC failed transfering "
			   " FOLLOW_UP", portnum(p), portnum(outgoing_port));
		goto out;
	}

	pr_debug("port %d to port %d: TC transfered "
		 " FOLLOW_UP", portnum(p), portnum(outgoing_port));

	msg_put(outgoing_msg);
	return 0;
...
/*Transparent clock extension -->*/
/*
 * Forward a SYNC message through the transparent clock.
 */
static int tc_forward_sync(struct port *p, 
			   struct ptp_message *m)
{
	struct port *outgoing_port;
	struct ptp_message *outgoing_msg;	
	...
	/* measure TC residence time in 2 steps (step 1)*/
	tc_residence_time_set(p->clock, tmv_zero());
	tc_residence_time(p, m->hwts.ts, 1);
	...
	err = port_prepare_and_send(outgoing_port, outgoing_msg, TRANS_EVENT);
	if (err) {
		pr_warning("port %d to port %d: TC failed transfering SYNC",
			   portnum(p), portnum(outgoing_port));
		goto out;
	}

	/* measure TC residence time in 2 steps (step 2)*/
	tc_residence_time(p, outgoing_msg->hwts.ts, 0);

	pr_debug("port %d to port %d: TC transfered SYNC", portnum(p),
		 portnum(outgoing_port));
...
//			 struct ptp_message *m)
/*Transparent clock extension -->*/
static int port_syfufsm(struct port *p, enum syfu_event event,
			struct ptp_message *m)
/*<--Transparent clock extension*/
{
...
/*Transparent clock extension -->*/
			if (clock_transparent(p->clock) && 
			    p->state == PS_TRANSPARENT) {
				if (tc_forward_sync(p, m)) {
					pr_err("TC failed during SYNC forwarding");
					return -1;
				}
			}
			p->syfu = SF_HAVE_SYNC;
...
/*Transparent clock extension -->*/
			if (clock_transparent(p->clock) && 
			    p->state == PS_TRANSPARENT) {
				if (tc_forward_follow_up(p, m)) {
					pr_err("TC failed during FOLLOW_UP forwarding");
					return -1;
				}
			}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	case PS_TRANSPARENT:
		add_foreign_master(p, m);
		/* In TC mode, force having a master without becoming a slave */
		result = 0;
		break;
/*<--Transparent clock extension*/
	}
...
/*Transparent clock extension -->*/
static int process_follow_up(struct port *p, struct ptp_message *m)
/*<--Transparent clock extension*/
{
...
calc:
	...
	//if (p->state == PS_UNCALIBRATED || p->state == PS_SLAVE) {
/*Transparent clock extension -->*/
	/* clock peer delay should be set even in transparent clock mode
	 * because it will be used to correct the originTs in the follow_up
	 * correction field BUT there is one value by port that share one clock
	 * (not a unique c->path_delay).
	 */
	if (p->state == PS_UNCALIBRATED || p->state == PS_SLAVE ||
	    (p->state == PS_TRANSPARENT && tc_port_side_get(p) == SIDE_MASTER)) {
		pr_debug("port %d: peerMeanPathDelay = %ld ns", 
			  portnum(p), TimeInterval_to_tmv(p->peerMeanPathDelay));
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
static int process_sync(struct port *p, struct ptp_message *m)
/*<--Transparent clock extension*/
{
	enum syfu_event event;
	struct PortIdentity master;
...
/*Transparent clock extension -->*/
		if (clock_transparent(p->clock)) {
			pr_err("a TC cannot work in ONE_STEP mode");
			exit(1);
		} else
			return 0;
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	case PS_TRANSPARENT:
		pr_err("transparent clock mode does not allow E2E delay "
		       "measurements");
		exit(1);
		break;
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	/* There seems to be no problem having jbod option AND being in 
	 * UNCALIBRATED state within a transparentclock */
	if (clock_transparent(p->clock))
		return;
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
		if (clock_transparent(p->clock)) {
			if (p->delayMechanism != DM_P2P) {
				pr_err("transparent clock mode "
				       "requires P2P measurements");
				return EV_FAULT_DETECTED;
			}
			return EV_NONE;
		}
...
/*Transparent clock extension -->*/
	/* Directly forward unchanged general messages through the TC */
	if (clock_transparent(p->clock) && p->state == PS_TRANSPARENT &&
	    (msg_type(msg) == ANNOUNCE || msg_type(msg) == SIGNALING
	    /*|| msg_type(msg) == MANAGEMENT*/
	    /* already done for MANAGEMENT with a limit of jumps */) ) {
		outgoing_port = tc_other_side_port(p);
		msg_pre_send(msg);
		if (port_forward(outgoing_port, msg)) {
			pr_err("TC failed forwarding %s", 
			       msg_type_string(msg_type(msg)));
			event = EV_FAULT_DETECTED;
		} else
			pr_debug("port %d to port %d: TC forwarded %s", 
				 portnum(p), portnum(outgoing_port), 
				 msg_type_string(msg_type(msg)));
...
/*Transparent clock extension -->*/
enum tc_port_side tc_port_side_get(struct port *p)
{
	return p->side;
}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
struct port *tc_other_side_port(struct port *p)
{
	if (clock_transparent(p->clock)) {
		if (tc_port_side_get(p) == SIDE_MASTER)
			return tc_port_by_side(SIDE_SLAVE, p->clock);
		else if (tc_port_side_get(p) == SIDE_SLAVE)
			return tc_port_by_side(SIDE_MASTER, p->clock);
		else
			pr_err("not possible to ask for an unknown side port");
	}
	else
		pr_err("not transparent clock has no sides");
	return NULL;
}
/*<--Transparent clock extension*/
...
/*Transparent clock extension -->*/
	if (clock_slave_only(clock) && clock_transparent(clock)) {
		pr_err("clock transparent and slave at the same time");
		goto err_port;
	} else if (clock_slave_only(clock))
		p->state_machine = ptp_slave_fsm;
	else if (clock_transparent(clock))
		p->state_machine = ptp_tc_fsm;
	else
...
/*Transparent clock extension -->*/
	/* set TC ports sides: always first as master and second as slave */
	if (clock_transparent(clock) && number != 0) {
		if (number == 1) {
			p->side = SIDE_MASTER;
			pr_notice("port %d: set as TC master side port (%s)"
				  "(/dev/ptp%d)", portnum(p), p->name,
				  p->iface->ts_info.phc_index);
		} else if (number == 2) {
			p->side = SIDE_SLAVE;
			pr_notice("port %d: set as TC slave side port (%s)"
				  "(/dev/ptp%d)", portnum(p), p->name,
				  p->iface->ts_info.phc_index);
		} else
			pr_err("incorrect port number for TC side assignment");
	} else
		p->side = SIDE_UNKNOWN;
/*<--Transparent clock extension*/
